...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::table('game_levels', function (Blueprint $table) {
            if (! Schema::hasColumn('game_levels', 'skill_focus')) {
                $table->string('skill_focus')->nullable()->after('target_position');
            }

            if (! Schema::hasColumn('game_levels', 'learning_objective')) {
                $table->text('learning_objective')->nullable()->after('skill_focus');
            }

            if (! Schema::hasColumn('game_levels', 'success_criteria')) {
                $table->text('success_criteria')->nullable()->after('learning_objective');
            }

            if (! Schema::hasColumn('game_levels', 'retry_hint')) {
                $table->text('retry_hint')->nullable()->after('success_criteria');
            }
        });
    ...
    public function down(): void
    {
        ...
        Schema::table('game_levels', function (Blueprint $table) {
            $columns = array_filter([
                Schema::hasColumn('game_levels', 'retry_hint') ? 'retry_hint' : null,
                Schema::hasColumn('game_levels', 'success_criteria') ? 'success_criteria' : null,
                Schema::hasColumn('game_levels', 'learning_objective') ? 'learning_objective' : null,
                Schema::hasColumn('game_levels', 'skill_focus') ? 'skill_focus' : null,
            ]);
