...
class AdminAiProviderEvaluationTest extends TestCase
{
 ...
 AiProviderLog::create([
 'provider_id' => $provider->id,
 'module' => 'attachment_text_extraction',
 'endpoint' => 'openai',
 ...
 ->assertOk()
 ->assertSee('OpenAI Process Connections')
 ->assertSee('Live API paths')
 ->assertSee('Safeguarded processes')
 ->assertSee('Question Generation')
 ->assertSee('Answer Evaluation and Feedback')
 ...
 ->assertSee('Readiness Band Classification')
 ->assertSee('Pronunciation Scoring')
 ->assertSee('Voice Delivery Scoring')
 ...
 ->assertJsonPath('total_provider_count', 4)
 ...
 return Http::response($this->providerJsonResponse($providerLabel, [
 'questions' => $questions,
 ]), 200);
 }
 ...
 ->assertSee('Cohere says the answer')
 ...
 [$cohere, 'cohere', 'Cohere', 'question_generation', 86, 91, 100, 700],
 [$openAi, 'openai', 'OpenAI', 'feedback_generation', 80, 78, 100, 850],
 [$groq, 'groq', 'Groq', 'feedback_generation', 86, 84, 100, 800],
 [$cohere, 'cohere', 'Cohere', 'feedback_generation', 97, 96, 100, 1400],
 ...
 $this->assertSame('cohere', $selector->bestProviderKeyForInterviewTask('feedback_generation'));
 }
 ...
 foreach ([[$groq, 'groq', 'Groq', 88], [$cohere, 'cohere', 'Cohere', 82]] as [$provider, $key, $name, $score]) {
 AiProviderEvaluationResult::create([
 'run_id' => $run->id,
 'provider_id' => $provider->id,
 'provider_key' => $key,
 ...
 'provider_name' => $providerName,
 'task_type' => $taskType,
 'case_key' => $taskType === 'question_generation'? 'user_request_questions_session_1': 'user_request_feedback_session_1',
 'status' => 'success',
 'response_time_ms' => $latency,
 'quality_score' => $quality,
 'reliability_score' => 95,
...
'status' => 'completed',
 ...
 private function providerJsonResponse(string $providerLabel, array $payload): array
 {
 $json = json_encode($payload);

 if ($providerLabel === 'Cohere') {
 return [
 'message' => [
 'content' => [[
 'type' => 'text',
