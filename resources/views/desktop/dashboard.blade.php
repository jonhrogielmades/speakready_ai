            ...
            <section id="card-recent-sessions" class="sr-card sr-card-pad sr-polished-card" style="--polish-accent:#06b6d4">
                ...
                @if(isset($recentSessions) && method_exists($recentSessions, 'hasPages') && $recentSessions->hasPages())
                    <div class="sr-recent-session-pager" aria-label="Recent sessions pagination">
                        <a
                            href="{{ $recentSessions->previousPageUrl() ?: '#' }}"
                            class="sr-recent-page-btn {{ $recentSessions->onFirstPage() ? 'disabled' : '' }}"
                            aria-disabled="{{ $recentSessions->onFirstPage() ? 'true' : 'false' }}"
                            @if($recentSessions->onFirstPage()) tabindex="-1" @endif
                        >
                            <i class="fa-solid fa-arrow-left"></i>
                            Previous
                        </a>
                        <span class="sr-recent-page-status">
                            Page {{ $recentSessions->currentPage() }} of {{ $recentSessions->lastPage() }}
                        </span>
                        <a
                            href="{{ $recentSessions->nextPageUrl() ?: '#' }}"
                            class="sr-recent-page-btn {{ $recentSessions->hasMorePages() ? '' : 'disabled' }}"
                            aria-disabled="{{ $recentSessions->hasMorePages() ? 'false' : 'true' }}"
                            @unless($recentSessions->hasMorePages()) tabindex="-1" @endunless
                        >
                            Next
