...
@push('styles')
<link rel="stylesheet" href="{{ asset('css/desktop/interview/setup.css?v=25') }}" data-page-style="interview-setup">
...
<div class="db-section active setup-step-mode" id="sec-interview-setup">
 ...
 <input class="oinp setup-input" type="text" name="target_position" id="valPosition" placeholder="{{ $targetFieldCopy['placeholder'] }}" value="{{ $targetPositionDefault }}" autocomplete="off" required aria-describedby="targetPositionError" aria-autocomplete="list" aria-controls="targetSuggestionList" aria-expanded="false" data-target-autocomplete-input>
 <div class="setup-target-suggestions" id="targetSuggestionList" role="listbox" aria-label="Suggested targets" hidden></div>
 </div>
...
<script>
 const setupTargetSuggestionEndpoint = @json(route('interview.targetSuggestions'));
 const setupTargetSuggestionCsrf = @json(csrf_token());
 ...
 let setupTargetSuggestionActiveIndex = -1;
 const setupAiTargetSuggestionState = {
 timer: null,
 controller: null,
 cache: new Map(),
 mode: null,
 ...
 function compactSetupScenarioText(value) {
 return normalizeSetupScenarioText(value).replace(/\s+/g, '');
 }

 function setupTargetSuggestionAcronym(value) {
 const keepWholeTokens = new Set(['abm', 'bs', 'gas', 'humss', 'ict', 'stem']);

 return normalizeSetupScenarioText(value)
 .split(' ')
 .filter(Boolean)
 .map((word) => keepWholeTokens.has(word)? word: word.charAt(0))
 .join('');
 }
 ...
 function currentSetupTargetMode() {
 return selectedSetupScenarioKind() === 'school'? 'school': 'job';
 }

 function normalizeSetupTargetSuggestionItem(item) {
 const value = String(typeof item === 'string'? item: (item?.value || item?.target || item?.position || item?.program || item?.text || '')).trim();
 ...
 function mergeSetupTargetSuggestionItems(primarySuggestions, fallbackSuggestions) {
 const seen = new Set();
 const merged = [];

 [...(primarySuggestions || []), ...(fallbackSuggestions || [])].forEach((item) => {
 const suggestion = normalizeSetupTargetSuggestionItem(item);
 if (!suggestion) return;

 const key = normalizeSetupScenarioText(suggestion.value);
 ...
 function setupTargetSuggestionItems() {
 const mode = currentSetupTargetMode();
 const query = normalizeSetupScenarioText(document.getElementById('valPosition')?.value);
 const fallbackSuggestions = setupTargetSuggestions[mode] || setupTargetSuggestions.job;

 if (
 setupAiTargetSuggestionState.mode === mode
 && setupAiTargetSuggestionState.query === query
 && setupAiTargetSuggestionState.items.length > 0
 ) {
 return mergeSetupTargetSuggestionItems(setupAiTargetSuggestionState.items, fallbackSuggestions);
 }

 return fallbackSuggestions;
 }

 function resetSetupAiTargetSuggestions() {
 if (setupAiTargetSuggestionState.timer) {
 window.clearTimeout(setupAiTargetSuggestionState.timer);
 }
 setupAiTargetSuggestionState.controller?.abort?.();
 setupAiTargetSuggestionState.timer = null;
 setupAiTargetSuggestionState.controller = null;
 setupAiTargetSuggestionState.mode = null;
 setupAiTargetSuggestionState.query = '';
 setupAiTargetSuggestionState.items = [];
 }

 function requestAiTargetSuggestions() {
 const input = document.getElementById('valPosition');
 if (!input || !setupTargetSuggestionEndpoint || typeof fetch !== 'function') return;

 const rawQuery = String(input.value || '').trim();
 const query = normalizeSetupScenarioText(rawQuery);
 const mode = currentSetupTargetMode();

 if (!query) {
 resetSetupAiTargetSuggestions();
 return;
 }

 const cacheKey = `${mode}:${query}`;
 const cached = setupAiTargetSuggestionState.cache.get(cacheKey);
 if (cached) {
 setupAiTargetSuggestionState.mode = mode;
 setupAiTargetSuggestionState.query = query;
 setupAiTargetSuggestionState.items = cached;
 renderSetupTargetSuggestions(false);
 return;
 }
 ...
 setupAiTargetSuggestionState.timer = window.setTimeout(() => {
 setupAiTargetSuggestionState.controller?.abort?.();
 const controller = new AbortController();
 setupAiTargetSuggestionState.controller = controller;

 fetch(setupTargetSuggestionEndpoint, {
 method: 'POST',
 headers: {
 'Accept': 'application/json',
 'Content-Type': 'application/json',
 'X-CSRF-TOKEN': setupTargetSuggestionCsrf,
 },
 body: JSON.stringify({
 category_id: document.getElementById('valScenario')?.value || null,
 query: rawQuery,
 limit: 7,
 }),
 ...
 ? data.suggestions.map(normalizeSetupTargetSuggestionItem).filter(Boolean).map((suggestion) => ({
 ...suggestion,
 aliases: Array.from(new Set([...(suggestion.aliases || []), rawQuery, query].filter(Boolean))),
 }))
 : [];
 setupAiTargetSuggestionState.cache.set(cacheKey, suggestions);

 if (currentSetupTargetMode() !== mode || normalizeSetupScenarioText(input.value) !== query) {
 return;
 }

 setupAiTargetSuggestionState.mode = mode;
 setupAiTargetSuggestionState.query = query;
 setupAiTargetSuggestionState.items = suggestions;
 renderSetupTargetSuggestions(false);
 })
 .catch((error) => {
 if (error?.name !== 'AbortError') {
 setupAiTargetSuggestionState.cache.set(cacheKey, []);
 }
 });
 }, 220);
 }

 function setupTargetSuggestionSearchTerms(suggestion) {
 const terms = new Set();

 [suggestion.value, ...(suggestion.aliases || [])].forEach((term) => {
 const normalized = normalizeSetupScenarioText(term);
 if (!normalized) return;

 const compact = compactSetupScenarioText(normalized);
 const acronym = setupTargetSuggestionAcronym(normalized);
 terms.add(normalized);
 if (compact.length > 1) terms.add(compact);
 if (acronym.length > 1) terms.add(acronym);
 });

 return Array.from(terms);
 }

 function setupTargetSuggestionScore(suggestion, query) {
 const normalizedQuery = normalizeSetupScenarioText(query);
 const compactQuery = compactSetupScenarioText(query);

 if (!normalizedQuery) return 0;

 return setupTargetSuggestionSearchTerms(suggestion).reduce((bestScore, term) => {
 const compactTerm = compactSetupScenarioText(term);
 const words = normalizeSetupScenarioText(term).split(' ').filter(Boolean);

 if (term === normalizedQuery || compactTerm === compactQuery) return Math.min(bestScore, 0);
 if (term.startsWith(normalizedQuery) || compactTerm.startsWith(compactQuery)) return Math.min(bestScore, 1);
 if (words.some((word) => word.startsWith(normalizedQuery))) return Math.min(bestScore, 2);
 if (normalizedQuery.length >= 2 && (term.includes(normalizedQuery) || compactTerm.includes(compactQuery))) return Math.min(bestScore, 3);
 if (normalizedQuery.length >= 3 && (normalizedQuery.includes(term) || compactQuery.includes(compactTerm))) return Math.min(bestScore, 4);

 return bestScore;
 }, Number.POSITIVE_INFINITY);
 }

 function setupTargetSuggestionMatches(suggestion, query) {
 return Number.isFinite(setupTargetSuggestionScore(suggestion, query));
 }

 function setupTargetSuggestionValueStartsQuery(suggestion, query) {
 const normalizedQuery = normalizeSetupScenarioText(query);
 const compactQuery = compactSetupScenarioText(query);
 const normalizedValue = normalizeSetupScenarioText(suggestion?.value);
 const compactValue = compactSetupScenarioText(suggestion?.value);

 return Boolean(
 normalizedQuery
 && (normalizedValue.startsWith(normalizedQuery) || compactValue.startsWith(compactQuery))
 );
 }

 function matchingSetupTargetSuggestions(showForEmpty = false) {
 const query = normalizeSetupScenarioText(document.getElementById('valPosition')?.value);
 const suggestions = setupTargetSuggestionItems();

 if (!query) {
 return showForEmpty? suggestions.slice(0, 6): [];
 }
 ...
 suggestion,
 index,
 score: setupTargetSuggestionScore(suggestion, query),
 valueStartsQuery: setupTargetSuggestionValueStartsQuery(suggestion, query)? 0: 1,
 rankWeight: currentSetupTargetMode() === 'job'? normalizeSetupScenarioText(suggestion.value).length: index,
 }))
 .filter((result) => Number.isFinite(result.score))
 .sort((first, second) => first.score - second.score || first.valueStartsQuery - second.valueStartsQuery || first.rankWeight - second.rankWeight || first.index - second.index)
 .map((result) => result.suggestion)
 .slice(0, 7);
 }

 function setupTargetSuggestionButtons() {
 return Array.from(document.querySelectorAll('#targetSuggestionList [data-target-suggestion]'));
 }

 function hideSetupTargetSuggestions() {
 const input = document.getElementById('valPosition');
 const panel = document.getElementById('targetSuggestionList');

 if (panel) panel.hidden = true;
 if (input) input.setAttribute('aria-expanded', 'false');
 setupTargetSuggestionActiveIndex = -1;
 }

 function setSetupTargetSuggestionActive(index) {
 const buttons = setupTargetSuggestionButtons();
 ...
 setupTargetSuggestionActiveIndex = (index + buttons.length) % buttons.length;
 buttons.forEach((button, buttonIndex) => {
 const isActive = buttonIndex === setupTargetSuggestionActiveIndex;
 button.classList.toggle('is-active', isActive);
 button.setAttribute('aria-selected', isActive? 'true': 'false');
 });
 }

 function chooseSetupTargetSuggestion(value) {
 const input = document.getElementById('valPosition');
 if (!input) return;
 ...
 hideSetupTargetSuggestions();
 input.focus();
 }

 function renderSetupTargetSuggestionLabel(label, value) {
 const query = String(document.getElementById('valPosition')?.value || '').trim();
 ...
 typed.textContent = value.slice(0, query.length);
 completion.textContent = value.slice(query.length);
 label.appendChild(typed);
 label.appendChild(completion);
 }

 function renderSetupTargetSuggestions(showForEmpty = false) {
 const input = document.getElementById('valPosition');
 const panel = document.getElementById('targetSuggestionList');

 if (!input || !panel || document.activeElement!== input) {
 hideSetupTargetSuggestions();
 return false;
 }

 const suggestions = matchingSetupTargetSuggestions(showForEmpty);
 panel.innerHTML = '';
 setupTargetSuggestionActiveIndex = -1;
 ...
 const iconClass = currentSetupTargetMode() === 'school'? 'fa-solid fa-graduation-cap': 'fa-solid fa-briefcase';
 ...
 button.type = 'button';
 button.className = 'setup-target-suggestion';
 button.dataset.targetSuggestion = suggestion.value;
 button.setAttribute('role', 'option');
 button.setAttribute('aria-selected', 'false');
 icon.className = iconClass;
 icon.setAttribute('aria-hidden', 'true');
 renderSetupTargetSuggestionLabel(label, suggestion.value);

 button.appendChild(icon);
 button.appendChild(label);
 button.addEventListener('mousedown', (event) => event.preventDefault());
 button.addEventListener('mouseenter', () => setSetupTargetSuggestionActive(index));
 button.addEventListener('click', () => chooseSetupTargetSuggestion(suggestion.value));
 ...
 const targetPositionInput = document.getElementById('valPosition');
 if (targetPositionInput) {
 targetPositionInput.addEventListener('focus', () => {
 renderSetupTargetSuggestions(true);
 requestAiTargetSuggestions();
 });
 ...
 if (document.getElementById('targetSuggestionList')?.hidden) {
 renderSetupTargetSuggestions(true);
 }
 ...
 setSetupTargetSuggestionActive(setupTargetSuggestionActiveIndex + (event.key === 'ArrowDown'? 1: -1));
 return;
 }

 if (event.key === 'Enter') {
 const buttons = setupTargetSuggestionButtons();
 const activeButton = buttons[setupTargetSuggestionActiveIndex];

 if (activeButton && !document.getElementById('targetSuggestionList')?.hidden) {
 event.preventDefault();
 chooseSetupTargetSuggestion(activeButton.dataset.targetSuggestion);
 }
 return;
 }
 ...
 targetPositionInput.addEventListener('blur', () => window.setTimeout(hideSetupTargetSuggestions, 120));
 }

 document.getElementById('valScenario')?.addEventListener('change', () => {
 resetSetupAiTargetSuggestions();
 renderSetupTargetSuggestions(Boolean(String(document.getElementById('valPosition')?.value || '').trim()));
 requestAiTargetSuggestions();
 });

 document.addEventListener('pointerdown', (event) => {
 if (!event.target?.closest?.('.setup-search-wrap')) {
 hideSetupTargetSuggestions();
 }
 });
