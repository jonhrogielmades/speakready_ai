...
@push('styles')
<link rel="stylesheet" href="{{ asset('css/desktop/user/feedback.css?v=15') }}" data-page-style="user-feedback">
@endpush
...
<div class="db-section active animate-fade-up feedback-shell">
 ...
 <p class="feedback-subtitle" style="color:rgba(248,251,255,.92)!important;-webkit-text-fill-color:rgba(248,251,255,.92)!important;">See what worked, what to improve, and what to practice next.</p>
 </div>
 </div>
 ...
 @else
 <span class="badge feedback-score-badge feedback-score-badge-pending" style="background: rgba(100, 116, 139, 0.15); color: var(--tx3);">Score pending</span>
 @endif
 </td>
 ...
 class="feedback-page-btn {{ $sessions->onFirstPage()? 'disabled': '' }}"
 aria-disabled="{{ $sessions->onFirstPage()? 'true': 'false' }}"
 @if($sessions->onFirstPage()) tabindex="-1" @endif
 >
 ...
 class="feedback-page-btn {{ $sessions->hasMorePages()? '': 'disabled' }}"
 aria-disabled="{{ $sessions->hasMorePages()? 'false': 'true' }}"
 @unless($sessions->hasMorePages()) tabindex="-1" @endunless
 >
 Next
