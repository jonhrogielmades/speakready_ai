...
<link rel="stylesheet" href="{{ asset('css/desktop/user/reports-2.css?v=10') }}" data-page-style="user-reports-2">
@endpush
...
@php
 ...
 $scoredInterviewCount = isset($scoredSessions)? $scoredSessions->count(): 0;
 $hasComparisonRows = count($comparisonRows?? []) > 0;
 $showProgressCharts = $scoredInterviewCount > 1;
@endphp
...
<div class="db-section active animate-fade-up" id="portfolioReport">
 ...
 <p style="color:var(--tx3);margin:0;">SpeakReady AI Interview Report &bull; Generated from {{ $reportGeneratedDate }}</p>
 </div>
 </div>
 ...
 <li>{{ $suggestion }}</li>
 @empty
 <li>Review your latest interview feedback</li>
 ...
 @if($sessions->count() > 0)
 You have completed interview records, but none of them have score data yet. Once a scored interview is available, this page will show your report summary, score breakdown, question analysis, improvement areas, and export options.
 @else
 Your report is generated automatically from scored interview performance. Complete your first practice interview to unlock your report summary, score breakdown, question analysis, improvement areas, and export options.
 @endif
 </p>
...
<script>
 ...
 @if($hasScoreData && $showProgressCharts)
 ...
 const rows = Array.from(table.querySelectorAll('tr')).map((row) => Array.from(row.querySelectorAll('th,td')).map((cell) => csvEscape(cell.textContent)).join(',')).join('\n');
