...
@section('title', 'Interview Challenges')

@push('styles')
<link rel="stylesheet" href="{{ asset('css/desktop/user/learning.css?v=1') }}" data-page-style="user-learning">
<link rel="stylesheet" href="{{ asset('css/desktop/user/learning-2.css?v=10') }}" data-page-style="user-learning-2">
@endpush
...
<div class="db-section active" id="learning-games-page">
 ...
 <div class="challenge-position-strip">
 <div class="challenge-position-summary">
 <span class="challenge-position-icon"><i class="fa-solid fa-user-tie"></i></span>
 <div>
 <div class="challenge-position-label">Target Position</div>
 <strong>{{ $selectedChallengePosition!== ''? $selectedChallengePosition: 'Choose your position' }}</strong>
 </div>
 </div>
 <button type="button" class="challenge-position-change-btn" data-bs-toggle="modal" data-bs-target="#challengePositionModal">
 <i class="fa-solid fa-pen-to-square"></i>
 <span>{{ $selectedChallengePosition!== ''? 'Change': 'Choose' }}</span>
 </button>
 </div>
 @if($usingGeneralChallengeFallback)
 <div class="challenge-position-note">
 <i class="fa-solid fa-circle-info"></i>
 <span>Showing broad interview challenges while role-specific levels for {{ $selectedChallengePosition }} are unavailable.</span>
 </div>
 @endif
 ...
 @endif
 @if($level->target_position)
 <span class="badge border" style="background:var(--bg3); color:var(--tx);"><i class="fa-solid fa-user-tie text-primary me-1"></i> {{ $level->target_position }}</span>
 @endif
...
<script>
 ...
 const positionModal = document.getElementById('challengePositionModal');
 ...
 { element: 'body:not(.modal-open) #modules-list', popover: { title: 'Challenge Path', description: 'Progress through active, completed, and locked levels to reach the final certificate reward.', side: 'top', align: 'start' }},
 { element: 'body:not(.modal-open) .level-node.active .level-card, body:not(.modal-open) #modules-list .level-node .level-card', popover: { title: 'Level Card', description: 'Review the level goal, skill focus, scoring target, and any constraints before starting.', side: 'top', align: 'center' }},
 { element: 'body:not(.modal-open) .active-challenge-panel', popover: { title: 'Success Checklist', description: 'Use these criteria as a quick plan for what your challenge answer must include.', side: 'top', align: 'center' }},
 { element: 'body:not(.modal-open) .start-challenge-btn', popover: { title: 'Start Challenge', description: 'Begin the active level when you have enough energy and are ready to answer.', side: 'top', align: 'center' }}
 ];
