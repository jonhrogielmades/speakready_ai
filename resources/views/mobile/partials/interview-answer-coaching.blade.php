@php
 ...
 $analysisStatus = is_array($coachingFeedback['analysis_status']?? null)? $coachingFeedback['analysis_status']: [];
 $deliveryCoaching = is_array($coachingFeedback['delivery']?? null)? $coachingFeedback['delivery']: [];
 $deliveryEvidence = is_array($deliveryCoaching['evidence']?? null)? $deliveryCoaching['evidence']: [];
 $cameraCoaching = is_array($coachingFeedback['camera']?? null)? $coachingFeedback['camera']: [];
 $cameraEvidence = is_array($cameraCoaching['evidence']?? null)? $cameraCoaching['evidence']: [];
 $questionCoaching = is_array($coachingFeedback['question']?? null)? $coachingFeedback['question']: [];
 $contentAlignment = is_array($coachingFeedback['content_alignment']?? null)? $coachingFeedback['content_alignment']: [];
 $feedbackQuality = is_array($coachingFeedback['feedback_quality']?? null)? $coachingFeedback['feedback_quality']: [];
 $alignmentEvidence = is_array($contentAlignment['evidence_quotes']?? null)? array_values(array_filter(array_map(fn ($item) => is_scalar($item)? trim((string) $item): '', $contentAlignment['evidence_quotes']))): [];
 ...
 $deliveryMetricItems = [];
 $durationSeconds = is_numeric($deliveryEvidence['duration_seconds']?? null)? (int) round($deliveryEvidence['duration_seconds']): null;
 $wordCount = is_numeric($deliveryEvidence['word_count']?? null)? (int) round($deliveryEvidence['word_count']): null;
 ...
 $fillerBreakdown =! $isUnavailableStatus($deliveryStatus) && is_array($deliveryEvidence['filler_breakdown']?? null)? array_values(array_filter($deliveryEvidence['filler_breakdown'], fn ($item) => is_array($item) && trim((string) ($item['word']?? ''))!== '' && (int) ($item['count']?? 0) > 0)): [];
 $fillerEvents =! $isUnavailableStatus($deliveryStatus) && is_array($deliveryEvidence['filler_events']?? null)? array_values(array_filter($deliveryEvidence['filler_events'], fn ($item) => is_array($item) && trim((string) ($item['word']?? ''))!== '' && is_numeric($item['at_seconds']?? null))): [];

 $cameraMetricItems = [];
 $cameraSampleCount = is_numeric($cameraEvidence['sample_count']?? null)? max(0, (int) round($cameraEvidence['sample_count'])): 0;
 ...
 $alignmentScore = is_numeric($contentAlignment['relevance_score']?? null)? max(0, min(100, (int) round($contentAlignment['relevance_score']))): null;
...
@if($hasStructuredCoaching)
 ...
 <div style="color:var(--tx3);font-size:.84rem;">What we saw <i class="fa-solid fa-arrow-right mx-1" aria-hidden="true"></i> Answer detail <i class="fa-solid fa-arrow-right mx-1" aria-hidden="true"></i> Next practice</div>
 </div>
 ...
 <div class="col-lg-4">
 <div class="h-100 p-3" style="background:var(--sf);border:1px solid var(--bd);border-radius:12px;">
 <div style="color:#8b5cf6;font-size:.78rem;font-weight:800;text-transform:uppercase;letter-spacing:.04em;margin-bottom:10px;"><i class="fa-solid fa-list-check me-2"></i>Next practice</div>
 ...
 <div><strong style="color:var(--tx2);">Speaking note:</strong> {{ $deliveryLimitation }}</div>
 @endif
 @if($cameraLimitation!== '')
 <div class="{{ $deliveryLimitation!== ''? 'mt-1': '' }}"><strong style="color:var(--tx2);">Camera note:</strong> {{ $cameraLimitation }}</div>
 @endif
 @if(!empty($cameraMetricItems))
