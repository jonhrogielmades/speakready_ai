...
@push('styles')
<link rel="stylesheet" href="{{ asset('css/mobile/admin/game.css?v=2') }}" data-page-style="admin-game">
@endpush
...
<div class="db-section active" id="sec-admin-game">
    <div class="mb-4 d-flex justify-content-between align-items-center admin-game-header">
        <div>
            <h4 style="font-size:1.4rem;font-weight:700;margin-bottom:4px">Interview Learning Games</h4>
            <p style="font-size:.875rem;color:var(--tx3);margin:0">Manage gamified interview drills, local practice prompts, and coaching modifiers.</p>
        </div>
        <div class="d-flex gap-2 admin-game-actions">
            <button class="btn btn-outline-info px-3 py-2" style="font-size:.85rem;font-weight:600" data-bs-toggle="modal" data-bs-target="#generateGameModal">
                <i class="fa-solid fa-wand-magic-sparkles me-1"></i> Generate Interview Games
            </button>
            <button class="bgrd btn px-3 py-2" style="font-size:.85rem" data-bs-toggle="modal" data-bs-target="#addGameModal">
                <i class="fa-solid fa-plus me-1"></i> Add Interview Game
            </button>
        </div>
    </div>
                        ...
                        <tr style="border-bottom:1px solid var(--bd); {{ $level->is_hidden ? 'opacity: 0.6;' : '' }}">
                            ...
                            <td style="padding:16px;vertical-align:middle;">
                                ...
                                <div class="d-flex flex-wrap align-items-center gap-2 mt-2" style="font-size:0.75rem;color:var(--tx2);">
                                    <span class="badge bg-secondary" style="font-size:0.65rem;">{{ $level->category ? $level->category->title : 'No Interview Category' }}</span> 
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-5" style="color:var(--tx3);">
                                ...
                                <h5>No Interview Games Found</h5>
                                <p>Click "Add Interview Game" or "Generate Interview Games" to create your first level.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="modal-content" style="border:1px solid var(--bd)">
            <form action="{{ route('admin.game.update', $level->id) }}" method="POST">
                ...
                <div class="modal-header" style="border-bottom:1px solid var(--bd)">
                    <h5 class="modal-title" style="color:var(--tx)">Edit Interview Game: Lvl {{ $level->level_number }}</h5>
                ...
                <div class="modal-body" style="max-height: 75vh; overflow-y: auto;">
                    ...
                    <div class="mb-4">
                        <label class="olbl">Interview Questions (For the user to answer)</label>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-4">
                            <label class="olbl">AI Persona (e.g. HR Coach)</label>
    ...
    <div class="modal-dialog">
        <div class="modal-content" style="border:1px solid var(--bd)">
            <form action="{{ route('admin.game.generate') }}" method="POST" id="generateGameForm">
                @csrf
                <div class="modal-header" style="border-bottom:1px solid var(--bd)">
                    <h5 class="modal-title" style="color:var(--tx)"><i class="fa-solid fa-wand-magic-sparkles text-info me-2"></i> Auto-Generate Interview Game</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" style="filter:invert(1)"></button>
                </div>
                <div class="modal-body">
                    <p style="color:var(--tx3); font-size:0.85rem;">Let the AI craft a interview practice level complete with local prompts, a coaching persona, modifiers, and rewards.</p>
                ...
                <div class="modal-footer" style="border-top:1px solid var(--bd)">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-info px-4 text-white" onclick="this.innerHTML='<i class=\'fa-solid fa-spinner fa-spin\'></i> Generating...';">Generate Interview Games</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content" style="border:1px solid var(--bd)">
            <form action="{{ route('admin.game.store') }}" method="POST">
                @csrf
                <div class="modal-header" style="border-bottom:1px solid var(--bd)">
                    <h5 class="modal-title" style="color:var(--tx)">Add Interview Game</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" style="filter:invert(1)"></button>
                </div>
                <div class="modal-body" style="max-height: 75vh; overflow-y: auto;">
                    <h6 style="color:var(--adm); border-bottom:1px solid var(--bd); padding-bottom:8px; margin-bottom:16px;">Core Settings</h6>
                    ...
                    <div class="mb-3">
                        <label class="olbl">Description</label>
                        <textarea class="oinp w-100" name="description" rows="2" placeholder="Brief description of the interview game/level"></textarea>
                    </div>
                    <div class="mb-4">
                        <label class="olbl">Interview Questions (For the user to answer)</label>
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-8">
                            <label class="olbl">Learning Objective</label>
                            <textarea class="oinp w-100" name="learning_objective" rows="2" placeholder="What interview skill should the learner improve?"></textarea>
                        </div>
                    </div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-4">
                            ...
                            <input class="oinp w-100" type="text" name="ai_persona" placeholder="e.g. HR Coach">
                        </div>
                        <div class="col-md-4">
                            <label class="olbl">Time Limit (Seconds)</label>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="oinp w-100" name="ai_custom_prompt" rows="2" placeholder="Hidden interview coaching instructions..."></textarea>
                    </div>
                ...
                <div class="modal-footer" style="border-top:1px solid var(--bd)">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="bgrd btn px-4">Create Interview Game</button>
                </div>
            </form>
        </div>
    </div>
