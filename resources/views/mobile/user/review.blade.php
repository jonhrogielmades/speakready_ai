...
@push('styles')
<link rel="stylesheet" href="{{ asset('css/mobile/user/review.css?v=2') }}" data-page-style="user-review">
@endpush
        ...
        <div class="col-12 animate-fade-up" style="animation-delay: 0.15s;">
            <div class="premium-panel" style="padding:24px;border:1px solid rgba(16,185,129,.22) !important;background:rgba(16,185,129,.04) !important;">
                <div class="d-flex flex-column flex-md-row justify-content-between gap-3 mb-3">
                    <div>
                        <h5 style="color:var(--tx);font-weight:800;margin-bottom:6px;"><i class="fa-solid fa-route me-2" style="color:#10b981"></i>After-Session Plan</h5>
        ...
        <div class="col-12 animate-fade-up" style="animation-delay: 0.18s;">
            <div class="premium-panel" style="padding:24px;">
                <div class="d-flex flex-column flex-md-row justify-content-between gap-2 mb-3">
                    <div>
                        <h5 style="color:var(--tx);font-weight:800;margin-bottom:6px;"><i class="fa-solid fa-user-pen me-2 text-primary"></i>Mentor or Peer Comments</h5>
                        <p style="color:var(--tx3);margin:0;font-size:.9rem;">Share this report and get comments from mentors, teachers, or peers.</p>
                    </div>
                    <button class="btn btn-outline-primary btn-sm" style="border-radius:999px;font-weight:800;" onclick="toggleShare()">
                        <i class="fa-solid fa-share-nodes me-1"></i>Get Link
                    </button>
                </div>
            ...
            <div id="collapse{{ $index }}" class="accordion-collapse collapse" data-bs-parent="#answersAccordion">
                <div class="accordion-body answer-review-body" style="border-top:1px solid var(--bd);padding:24px;">
                    ...
                    @else
                        ...
                        {{-- Old visual estimates are shown for history, never as readiness score data. --}}
                                    ...
                                    <div class="d-flex flex-column flex-md-row justify-content-between gap-2" style="color:var(--tx);border-bottom:1px solid var(--bd);padding-bottom:10px;">
                                        ...
                                        <div class="retry-meta">
                                            ...
                                            @if(in_array(strtolower((string) $retry->response_mode), ['voice', 'hybrid', 'voice_and_text'], true) && ($retry->voice_duration ?? 0) > 0 && $retry->delivery_stability_score !== null)
                                                <span class="retry-chip">Speaking Steadiness {{ $retry->delivery_stability_score }}%</span>
                                            @endif
                                        </div>
                                    </div>
                    ...
                    <div class="answer-retry-action">
                        ...
                        <div class="retry-panel" id="retry-panel-{{ $answer->id }}" data-url="{{ route('interview.answer.retry', $answer->id) }}">
                            <div class="d-flex flex-column flex-md-row justify-content-between gap-2 mb-3">
                                <div>
                                    <strong style="color:var(--tx);">Practice Try</strong>
                                    <div style="color:var(--tx3);font-size:.85rem;">This saves as a retry and does not change the original score.</div>
                                </div>
                                <div class="retry-meta">
                            ...
                            <textarea class="oinp retry-textarea" id="retry-text-{{ $answer->id }}" rows="5" style="font-size:.95rem;" placeholder="Record or type your better answer here..." onfocus="startRetryTimer({{ $answer->id }})" oninput="updateRetryWordCount({{ $answer->id }})"></textarea>
                            <div class="d-flex flex-column flex-md-row gap-2 mt-3">
                                ...
                                <button type="button" class="btn btn-primary" style="border-radius:12px;font-weight:700;" onclick="submitRetry({{ $answer->id }})">
                                    <i class="fa-solid fa-paper-plane me-1"></i>Submit Try
                                </button>
                            </div>
...
function retryCoachingHtml(coaching) {
    ...
    if (delivery.observation) {
        rows.push(`<p style="margin:0 0 8px;color:var(--tx2);line-height:1.55;"><strong style="color:var(--tx);">Speaking (${retryEscape(String(delivery.status || 'not measured').replace(/_/g, ' '))}):</strong> ${retryEscape(delivery.observation)}</p>`);
    }
    if (question.tip) {
    ...
    return `<div class="mt-3 p-3" style="background:rgba(59,130,246,.06);border:1px solid rgba(59,130,246,.2);border-radius:10px;"><strong style="display:block;color:#3b82f6;margin-bottom:8px;">Retry coaching</strong>${rows.join('')}</div>`;
}
...
function submitRetry(answerId) {
    ...
    if (!text) {
        ...
        result.innerHTML = '<div class="alert alert-warning mb-0">Please enter your better answer first.</div>';
        return;
    }
    ...
    .then(data => {
        ...
        const deliveryChip = data.delivery_stability_score === null || data.delivery_stability_score === undefined
            ? ''
            : `<span class="retry-chip">Speaking Steadiness ${retryEscape(data.delivery_stability_score)}%</span>`;
