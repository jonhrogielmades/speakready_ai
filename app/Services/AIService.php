...
EOT;
    ...
    public static function evaluateAiCollaboration(array $sessionData, array $conversationLog, string $finalRecommendation, string $reflection = '', $provider = 'gemini'): array
    {
        if (strtolower(trim((string) $provider)) === 'local') {
            return self::fallbackAiCollaborationEvaluation($conversationLog, $finalRecommendation, $reflection);
        }

        $conversationText = collect($conversationLog)
            ->filter(fn ($item) => is_array($item) && isset($item['role'], $item['content']))
            ->map(fn ($item) => strtoupper((string) $item['role']) . ': ' . trim((string) $item['content']))
            ->implode("\n\n");

        $prompt = "You are an interview assessor evaluating how a candidate collaborated with an AI assistant during a workplace simulation.\n";
        $prompt .= "Score the candidate's AI collaboration skill, not just the final business answer.\n\n";
        $prompt .= "Simulation Title: " . ($sessionData['title'] ?? 'AI Collaboration Simulation') . "\n";
        $prompt .= "Target Role: " . ($sessionData['role'] ?? 'General role') . "\n";
        $prompt .= "Industry/Context: " . ($sessionData['industry'] ?? 'General business') . "\n";
        $prompt .= "Difficulty: " . ($sessionData['difficulty'] ?? 'medium') . "\n\n";
        $prompt .= "Scenario Brief:\n" . self::truncateText($sessionData['scenario_brief'] ?? '', 600) . "\n\n";
        $prompt .= "Source Material:\n" . self::truncateText($sessionData['source_material'] ?? '', 500) . "\n\n";
        $prompt .= "Expected Output:\n" . self::truncateText($sessionData['expected_output'] ?? '', 200) . "\n\n";
        $prompt .= "AI Collaboration Transcript:\n" . self::truncateText($conversationText, 900) . "\n\n";
        $prompt .= "Candidate Final Recommendation:\n" . self::truncateText($finalRecommendation, 900) . "\n\n";
        $prompt .= "Candidate Reflection:\n" . self::truncateText($reflection, 400) . "\n\n";

        $prompt .= <<<EOT
Return ONLY a valid JSON object. Do not include Markdown or text outside JSON.
...
OUTPUT SCHEMA:
{
  ...
  "strengths": "1-2 evidence-based sentences.",
  "improvements": "1-2 specific improvement sentences.",
  "next_drill": "One concrete practice drill.",
  "evidence_notes": "What in the transcript or final answer supports your scoring.",
  "ethics_note": "A short note about using AI as preparation and not as hidden real-time answer generation."
}

SCORING RUBRIC:
- prompt_quality_score: clear asks, context, constraints, iteration, and useful follow-up prompts.
- critical_thinking_score: challenges AI output, compares options, names assumptions, rejects weak reasoning.
- verification_score: identifies missing evidence, metrics, risks, checks, or validation steps.
- structure_score: final recommendation has a clear decision, evidence, tradeoffs, risks, and next step.
- communication_score: concise, professional, role-appropriate, easy to present in an interview.

Score harshly if the candidate lets AI make the decision, copies vague AI output, ignores source material, or provides no verification plan.
EOT;

        foreach (self::feedbackProviderPriority($provider) as $currentProvider) {
            try {
                $response = self::callStructuredProvider($currentProvider, $prompt);
                if (!empty($response)) {
                    return self::normalizeAiCollaborationEvaluation($response, $conversationLog, $finalRecommendation, $reflection);
                }
            } catch (\Exception $e) {
                Log::error("AI Collaboration Evaluation Error ({$currentProvider}): " . $e->getMessage());
            }
        }
    ...
    private static function normalizeAiCollaborationEvaluation(array $response, array $conversationLog, string $finalRecommendation, string $reflection): array
    {
        $fallback = self::fallbackAiCollaborationEvaluation($conversationLog, $finalRecommendation, $reflection);
        $fields = [
        ...
        foreach ($fields as $field) {
            $normalized[$field] = array_key_exists($field, $response)
                ? self::normalizeScore($response[$field])
                : $fallback[$field];
        }

        $average = (int) round(array_sum(array_values($normalized)) / count($normalized));
        $normalized['overall_score'] = array_key_exists('overall_score', $response)
            ? self::normalizeScore($response['overall_score'])
            : $average;

        foreach (['strengths', 'improvements', 'next_drill', 'evidence_notes', 'ethics_note'] as $field) {
            $text = self::feedbackText($response[$field] ?? '');
            $normalized[$field] = $text !== '' ? $text : $fallback[$field];
        }

        return $normalized;
    }

    private static function fallbackAiCollaborationEvaluation(array $conversationLog, string $finalRecommendation, string $reflection): array
    {
        $userPrompts = array_values(array_filter($conversationLog, fn ($item) => ($item['role'] ?? '') === 'user'));
        $promptText = strtolower(implode(' ', array_map(fn ($item) => (string) ($item['content'] ?? ''), $userPrompts)));
        $finalText = strtolower($finalRecommendation . ' ' . $reflection);
        $wordCount = str_word_count($finalRecommendation);

        $promptSignals = self::signalCount($promptText, ['compare', 'risk', 'assumption', 'evidence', 'metric', 'alternative', 'verify', 'tradeoff', 'prioritize']);
        $thinkingSignals = self::signalCount($finalText, ['because', 'tradeoff', 'risk', 'assumption', 'alternative', 'however', 'therefore']);
        $verificationSignals = self::signalCount($finalText, ['verify', 'validate', 'test', 'measure', 'metric', 'data', 'evidence', 'check']);
        $structureSignals = self::signalCount($finalText, ['recommend', 'first', 'because', 'risk', 'next', 'measure', 'option']);

        $promptQuality = min(100, 35 + (count($userPrompts) * 12) + ($promptSignals * 6));
        $criticalThinking = min(100, 38 + ($thinkingSignals * 8));
        $verification = min(100, 35 + ($verificationSignals * 10));
        $structure = min(100, 40 + ($structureSignals * 8) + ($wordCount >= 80 ? 10 : 0));
        $communication = min(100, max(35, 55 + ($wordCount >= 70 ? 18 : 0) + ($wordCount <= 260 ? 12 : -8)));

        if ($wordCount < 45) {
            $structure = min($structure, 55);
            $communication = min($communication, 58);
        }
        ...
        return array_merge([
            ...
            'strengths' => 'The recommendation was evaluated with local scoring because AI evaluation was unavailable. Stronger signals came from prompts and final-answer language that mentioned evidence, risks, assumptions, or metrics.',
            'improvements' => 'Ask the assistant to compare alternatives, challenge assumptions, and identify missing evidence before writing the final answer.',
            'next_drill' => 'Run another simulation and ask three prompts: one for options, one for risks, and one for verification metrics before deciding.',
            'evidence_notes' => 'Local scoring considered prompt count, use of reasoning words, verification language, final-answer length, and recommendation structure.',
            'ethics_note' => 'Use AI to practice and sharpen judgment before an interview, not to secretly generate live answers during an assessment.',
        ]);
    }

    private static function signalCount(string $text, array $signals): int
    {
        $count = 0;
