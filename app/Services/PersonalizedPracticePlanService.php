...
class PersonalizedPracticePlanService
{
 ...
 public function forUser(int $userId, int $limit = 4, ?Collection $moduleRecommendations = null): Collection
 {
 ...
 $moduleRecommendations = $modulesEnabled? ($moduleRecommendations?? $this->recommendations->forUser($userId, 3)): collect();
 ...
 $primaryArea->icon,
 $primaryArea->color,
 $coachEnabled? ['Ask the coach for one sample answer', 'Practice the answer aloud once']: ['Answer one focused question', 'Apply the practice tip immediately']
 );
 }
 ...
 'Start Your Baseline',
 'Readiness',
 'Complete a short mock interview to personalize your next steps.',
 ...
 $focus,
 $primaryArea? "Practice one answer aloud with extra attention to {$primaryArea->label}.": 'Practice one answer aloud, then write the sentence you would improve.',
 ...
 $scenario = $latestSession?->category?->title?: 'interview';
 ...
 $primaryArea? "Use this round to improve {$primaryArea->label}, your lowest recent area.": 'A focused mock interview gives the plan fresher data.',
 20,
 route('interview.setup'),
 'Start Interview',
 'fa-microphone-lines',
 ...
 {
 return $this->number($latestSession?->score?->overall_readiness_score)?? $this->number($profile?->readiness_score);
 }
