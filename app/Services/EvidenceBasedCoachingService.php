 ...
 */
 public function normalizeObservationData(?array $clientData,
 string $answerText,
 array $metrics,
 bool $cameraEnabled
 ): array {
 ...
 'pause_count' => $deliveryMeasured? $this->boundedInt($metrics['pause_count']?? 0, 0, 500): null,
 'filler_total' => null,
 ...
 'filler_events' => [],
 'caveat' => $deliveryMeasured? 'Words and possible filler words come from the saved voice text. Time and pause counts are browser estimates. Voice text may miss hesitation sounds, and some matched words may still make sense.': 'Speaking was not measured because this answer does not have both a recorded time and usable voice text.',
 ];
 ...
 $contentStatus = $contentWordCount === 0? 'unscored': ($contentWordCount < 10? 'limited_evidence': (((int) ($metrics['scoring_confidence']?? 0)) > 0? 'scored': 'limited_evidence'));
 ...
 $storedObservation = is_array($answer->observation_data?? null)? $answer->observation_data: [];
 $storedCamera = is_array($storedObservation['camera']?? null)? $storedObservation['camera']: [];
 ...
 && (int) ($answer->coaching_feedback['version']?? 0) >= self::VERSION? $answer->coaching_feedback: $this->forAnswer(
 $this->answerContent($answer),
 $answer->question,
 $metrics,
 $normalized
 );
 $answerQuality = is_array($coaching['feedback_quality']?? null)? $coaching['feedback_quality']: [];
 ...
 && in_array($alignmentStatus, ['directly_answered', 'partially_answered', 'low_relevance'], true)? max(0, min(100, (int) round($alignment['relevance_score']))): null;
 ...
 'severity' => $issueCode === 'missing_criteria'? 75: $this->alignmentSeverity($alignmentStatus),
 'affected_count' => 0,
 'questions' => [],
 'question_ids' => [],
 'evidence_quotes' => [],
 ...
 $priority['severity'] = is_numeric($priority['severity']?? null)? max(0, min(100, (int) round($priority['severity']))): $this->prioritySeverity($area);
 ...
 $observations[] = $fillerTotal > 0? "Across {$coverage['delivery_measured']} recorded answers, the saved voice text found {$fillerTotal} possible filler words in {$deliveryWords} words.": "Across {$coverage['delivery_measured']} recorded answers, no saved filler words were found in the voice text.";
 } else {
 ...
 $observations[] = "Optional camera detection had enough checks for {$coverage['camera_measured']} answers: a face was seen in {$visibility}% of checks, the head looked camera-facing in {$facing}% of face checks, and body position was seen in ".((int) round(($cameraPoseDetected / max(1, $cameraSamples)) * 100)).'% of checks.';
 if ($cameraShouldersMeasured > 0 || $cameraUprightMeasured > 0 || $cameraMovementMeasured > 0) {
 $observations[] = "Camera notes across checked parts: shoulders looked level in {$shoulders}% of body checks, upper body looked upright in {$upright}% of body checks, and higher movement appeared in {$highMovement}% of movement checks.";
 }
 ...
 $focusHeadline = $priorityActions!== []? 'Top focus: '.trim((string) ($priorityActions[0]['area']?? 'answer practice')).'.': 'No repeated gap was found. Keep adding true details that answer the question.';
 $feedbackQualityPercent = $feedbackChecksTotal > 0? (int) round(($feedbackChecksPassed / $feedbackChecksTotal) * 100): 0;
 ...
 'coverage' => $coverage,
 'feedback_quality' => [
 'status' => $feedbackChecksTotal === 0? 'not_available': ($feedbackQualityPercent === 100? 'verified': 'limited'),
 ...
 $observation = $total > 0? "The saved voice text found {$total} possible filler words in {$wordCount} words ({$rate} per 100 words)".($breakdownText!== ''? ": {$breakdownText}.": '.'): "No saved filler words were found in the {$wordCount}-word voice text.";
 ...
 data_get($analysis, 'gop.score')?? data_get($analysis, 'pronunciation.score'),
 0,
 100
 );
 $status = in_array((string) ($analysis['status']?? ''), ['measured', 'partial'], true)? (string) $analysis['status']: ($score!== null? 'partial': 'not_measured');
 ...
 $limitation = $limitations!== []? implode(' ', array_slice(array_map(fn ($item): string => trim((string) $item), $limitations), 0, 3)): 'Local speech models were not available or not configured for this answer.';
 ...
 $componentText = $measuredComponents!== []? ' Local checks used: '.implode(', ', array_map(fn ($item): string => (string) $item, $measuredComponents)).'.': '';
 $observation = $score!== null? "A local speech check gave a pronunciation score of {$score}%.": 'A local speech check ran, but it did not give a pronunciation score.';
 ...
 $tips[] = 'Keep movement steady and return to a neutral position between main points.';
 }
 if ($tips === []) {
 ...
 $observation = $status === 'insufficient_data'? 'Optional browser camera samples did not cover enough of the answer for a clear camera note.': (isset($reasonLabels[$unavailableReason])? 'Optional camera detection was not measured because '.$reasonLabels[$unavailableReason].'.': 'Optional camera detection was not measured for this answer.');

 return [
 'status' => $status === 'insufficient_data'? 'insufficient_data': 'not_measured',
 'observation' => $observation,
 'tip' => 'For camera detection, use steady front light and keep your face and shoulders in the preview when possible.',
 'tips' => ['For camera detection, use steady front light and keep your face and shoulders in the preview when possible.'],
 ...
 {
 $text = trim((string) ($question instanceof Question? $question->question_text: ($question['question_text']?? $question['question']?? '')));
 $expectedGuide = trim((string) ($question instanceof Question? $question->expected_guide: ($question['expected_guide']?? '')));
 $mappedSkills = $question instanceof Question? $question->mapped_skills: ($question['mapped_skills']?? []);
 ...
 */
 ...
 $questionText = trim((string) ($question instanceof Question? $question->question_text: ($question['question_text']?? $question['question']?? '')));
 $questionId = $question instanceof Question? $question->getKey(): ($question['id']?? $question['question_id']?? null);
 ...
 $status = match (true) {
 $isSkipped => 'skipped',
 $isTooShort => 'insufficient_evidence',! $hasScore => 'not_evaluated',
 ...
 $missingPoints = $coverageTargets['missing_points']!== []? $coverageTargets['missing_points']: ['The answer did not give enough clear detail to check it well.'];
 ...
 $insufficientAction = $coverageTargets['action']!== ''? $coverageTargets['action']: 'Expand the answer with enough specific, question-relevant detail to assess it.';
 $action = match ($status) {
 'skipped' => "Practice a direct response to {$questionLabel}. {$guidance}",
 'insufficient_evidence' => trim($insufficientAction.' '.$guidance),
 'directly_answered' => $gap? "Keep the direct answer to {$questionLabel}, then cover this missing point: {$gap}": "Keep the direct opening for {$questionLabel}, then strengthen it with the most useful true detail or result.",
 ...
 'directly_answered' => $evidenceExcerpt? 'The answer covered the main focus and gave this useful detail: "'.$evidenceExcerpt.'".': 'The answer covered the main focus directly.',
 'partially_answered' => $evidenceExcerpt? 'The answer made a useful start with: "'.$evidenceExcerpt.'".': 'The answer had a useful starting point.',
 'low_relevance' => $evidenceExcerpt? 'A complete answer was sent, and this part gives you useful text to improve: "'.$evidenceExcerpt.'".': 'An answer was sent, giving you useful text to improve.',
 'insufficient_evidence' => $evidenceExcerpt? 'Only limited answer detail was available: "'.$evidenceExcerpt.'". Add the missing details before trusting this review.': 'An answer was started, but it needs more useful detail before it can be checked well.',
 ...
 'directly_answered' => $gap? $gap: 'Keep the direct answer and make the strongest truthful supporting detail easier to identify.',
 'partially_answered', 'low_relevance' => $gap?: $guidance,
 'insufficient_evidence' => $coverageTargets['improvement_focus']!== ''? $coverageTargets['improvement_focus']: 'Add enough clear detail that fits the question.',
 ...
 'insufficient_evidence' => $coverageTargets['next_attempt_steps']!== []? $coverageTargets['next_attempt_steps']: array_merge(
 ...
 'insufficient_evidence' => $coverageTargets['success_check']!== ''? $coverageTargets['success_check']: 'The retry has enough clear detail to explain the answer.',
 ...
 'relevance_score' => in_array($status, ['directly_answered', 'partially_answered', 'low_relevance'], true)? $score: null,
 'observation' => $observation,
 ...
 $evidenceQuotes = is_array($contentAlignment['evidence_quotes']?? null)? array_values(array_filter($contentAlignment['evidence_quotes'], fn ($quote): bool => is_string($quote) && trim($quote)!== '')): [];
 ...
 'score_uncertainty_reported' => in_array($status, $scoredStatuses, true)? is_numeric($contentAlignment['scoring_confidence']?? null):! is_numeric($contentAlignment['relevance_score']?? null),
 ...
 $nextSteps[] = $mentionsCleanup? 'List the cleanup steps you personally took in order.': 'Describe the specific steps you took.';
 ...
 $nextSteps[] = $mentionsCleanup? 'Name the cleaning tools, supplies, PPE, or agents used.': 'Name the tools, supplies, equipment, or materials involved.';
 ...
 $nextSteps[] = $mentionsCleanup? 'Close with the finished result, safety check, or lesson learned.': 'Close with the true result, impact, or lesson.';
 $actionParts[] = $mentionsCleanup? 'state the finished result or safety check': 'state the true result or lesson';
 ...
 $action = $actionParts === []? '': 'Make the answer longer with specific, true details: '.$this->readableList($actionParts).'.';
 $improvementFocus = $successParts === []? '': 'Add '.$this->readableList($successParts).' so the answer can be checked well.';
 $successCheck = $successParts === []? '': 'The retry names '.$this->readableList($successParts).' clearly enough to check.';
 ...
 $shouldersLevel = $shouldersVisible? $this->nullableBoolean($sample['shoulders_level']?? null): null;
 $uprightPosture = $poseDetected? $this->nullableBoolean($sample['upright_posture']?? null): null;
 ...
 'samples' => $normalized,
 'source' => $hasBodySignals? 'browser_reported_pose_landmark_estimate': 'browser_reported_landmark_estimate',
