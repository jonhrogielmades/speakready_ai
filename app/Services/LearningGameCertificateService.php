...
{
 public function completedPathStats(User $user, Category $category,?string $targetPosition = null):?array
 {
 $levels = $this->certificateLevels($category, $targetPosition);
 if ($levels->isEmpty()) {
 return null;
 }
 ...
 public function isUnlocked(User $user, Category $category,?string $targetPosition = null): bool
 {
 return $this->completedPathStats($user, $category, $targetPosition)!== null;
 }

 public function issueFor(User $user, Category $category,?string $targetPosition = null): GameCertificate
 {
 $stats = $this->completedPathStats($user, $category, $targetPosition);
 ...
 public function pdfBytes(GameCertificate $certificate, User $user, Category $category,?string $targetPosition = null): string
 {
 $stats = $this->completedPathStats($user, $category, $targetPosition);
 abort_unless($stats, 403);
 ...
 [$category->title?: 'Interview Challenges', 420, 286, 22, 'F2', [0.08, 0.34, 0.39], true],
 ...
 private function certificateLevels(Category $category,?string $targetPosition = null): Collection
 {
 $levels = GameLevel::where('category_id', $category->id)
 ...
 $targetPosition = $challengePositions->clean($targetPosition);

 $relatedLevels = $targetPosition!== ''? $challengePositions->relatedLevels($levels, $targetPosition): $levels;

 return $challengePositions->journeyLevels($relatedLevels);
 }
