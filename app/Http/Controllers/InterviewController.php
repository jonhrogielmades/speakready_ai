...
class InterviewController extends Controller
{
 ...
 $position = trim((string) $validated['target_position']);
 if ($position === 'Other' &&! empty($validated['custom_position'])) {
 $position = trim((string) $validated['custom_position']);
 }

 if ($message = $this->scenarioTargetMismatchMessage($category, $position)) {
 return back()
 ->withErrors(['target_position' => $message])
 ->withInput();
 }
 ...
 $this->interviewFocusForCategory($category, $validated['interview_focus']?? null)
 );
 $pressureMode = false;
 ...
 $savedAnswerInput = $this->answerContentForAssessment($answer);
 if ($interviewerInputText === '' && $savedAnswerInput!== '') {
 $interviewerInputText = $savedAnswerInput;
 }
 if ($interviewerInputText === '' && $this->answerHasStoredVoiceRecording($answer)) {
 ...
 try {
 $followUpText = $followUpEnabled? AIService::generateChatReply($session, $history, $interviewerInputText, $provider, $isFinal, $this->currentLanguageConfig(), $conversationContext, $dataset): AIService::fallbackInterviewReply($session, $history, $interviewerInputText, $isFinal);
 } catch (\Throwable $error) {
 ...
 $transcription = $this->transcribeUploadedSpeechAnswer(
 $validated['audio'],
 $session,
 $question,
 ...
 private function transcribeUploadedSpeechAnswer(
 ...
 $this->transcriptionContext($session, $question, $previousTranscript)
 );
 ...
 'retry_after_ms' => 1200,
 ], 409): abort(409, 'Feedback analysis is already in progress.');
 }
 ...
 $message = $providerFailure? $error->userMessage(): 'Your answers were saved, but the feedback report could not be finalized. Please retry the report generation in a moment.';

 return $request->expectsJson()? response()->json(array_filter([
 'message' => $message,
 'error_code' => $providerFailure? 'ai_feedback_providers_failed': null,
 'provider_count' => $providerFailure? $error->providerCount(): null,
 'providers_attempted' => $providerFailure? $error->attemptedProviders(): null,
 'retry_after_ms' => 1500,
 ], fn ($value) => $value!== null), 503): back()->with('error', $message);
 }
 }
 ...
 || app(FeedbackCoachingRepair::class)->summaryNeedsRepair($session->feedback?->coaching_summary?? null)
 || $this->completedSessionHasVoiceFeedbackEvidenceGap($session);
 }

 private function completedSessionScoreIsCurrent(InterviewSession $session): bool
 {
 $session->loadMissing('score');
 $rubric = is_array($session->score?->rubric?? null)? $session->score->rubric: [];
 ...
 private function completedSessionHasVoiceFeedbackEvidenceGap(InterviewSession $session): bool
 {
 if (! AIService::speechTranscriptionAvailable()) {
 return false;
 }

 return InterviewAnswer::where('interview_session_id', $session->id)
 ->whereNull('retry_of_answer_id')
 ->whereNotNull('voice_recording_path')
 ->where('voice_recording_path', '!=', '')
 ->where(function ($query) {
 $query->whereNull('answer_text')
 ->orWhere('answer_text', '');
 })
 ->where(function ($query) {
 $query->whereNull('delivery_transcript')
 ->orWhere('delivery_transcript', '');
 })
 ->where(function ($query) {
 $query->whereNull('voice_recording_transcription_status')
 ->orWhereNotIn('voice_recording_transcription_status', [
 ...
 InterviewAnswerSchema::ensure();
 $answerText??= $this->cleanTranscribedAnswer($validated['answer_text']?? '');
 $validated = $this->withSubmittedVoiceFeedbackTranscript($session, $question, $validated, $answerText);
 ...
 private function withSubmittedVoiceFeedbackTranscript(
 InterviewSession $session,
 Question $question,
 array $validated,
 string $answerText
 ): array {
 if (trim($answerText)!== '' ||! $this->hasSubmittedVoiceRecording($validated)) {
 return $validated;
 }

 if ($this->cleanTranscribedAnswer($validated['speech_transcript']?? '')!== '') {
 ...
 try {
 $transcription = $this->transcribeUploadedSpeechAnswer($upload, $session, $question);
 } catch (\Throwable $error) {
 Log::warning('Voice answer feedback transcription failed before save.', [
 'session_id' => $session->id,
 'question_id' => $question->id,
 'error_type' => $error::class,
 ...
 $transcript = $this->cleanTranscribedAnswer($transcription['transcript']?? null);
 if ($transcript!== '') {
 $validated['speech_transcript'] = $transcript;
 $validated['voice_recording_transcription_status'] = 'transcribed';
 } else {
 $validated['voice_recording_transcription_status'] = $this->feedbackTranscriptionStatusFrom($transcription)?: ($validated['voice_recording_transcription_status']?? null);
 }

 if (! filled($validated['pronunciation_analysis']?? null) && is_array($transcription['pronunciation_analysis']?? null)) {
 $validated['pronunciation_analysis'] = json_encode(
 $transcription['pronunciation_analysis'],
 ...
 private function ensureVoiceAnswersHaveFeedbackEvidence(InterviewSession $session, Collection $answers): Collection
 {
 if ($answers->isEmpty() ||! AIService::speechTranscriptionAvailable()) {
 return $answers;
 }

 return $answers->map(function ($answer) use ($session) {
 if (! $answer instanceof InterviewAnswer
 || $this->answerContentForAssessment($answer)!== ''
 ||! $this->answerHasStoredVoiceRecording($answer)
 ||! ($answer->question instanceof Question)) {
 return $answer;
 }

 $upload = $this->uploadedFileForStoredVoiceRecording($answer);
 if (! $upload) {
 return $answer;
 }

 try {
 $transcription = $this->transcribeUploadedSpeechAnswer($upload, $session, $answer->question);
 } catch (\Throwable $error) {
 Log::warning('Stored voice answer feedback transcription failed.', [
 'session_id' => $session->id,
 'answer_id' => $answer->id,
 'question_id' => $answer->question_id,
 'error_type' => $error::class,
 ...
 if ($transcript === '') {
 $answer->forceFill([
 'voice_recording_transcription_status' => $this->feedbackTranscriptionStatusFrom($transcription)?: $answer->voice_recording_transcription_status,
 ])->save();

 return $answer->refresh()->load('question');
 }

 $responseMode = $this->isVoiceResponseMode($answer->response_mode?? $session->response_mode?? 'voice')? $this->normalizeResponseMode($answer->response_mode?? $session->response_mode?? 'voice'): 'voice';
 $duration = $this->clampInt($answer->voice_duration?? 0, 0, 7200);
 $deliveryMetrics = $this->deliveryMetricsFrom([
 'response_mode' => $responseMode,
 'voice_duration' => $duration,
 'pause_count' => $answer->pause_count?? 0,
 ], $transcript);
 $pronunciationAnalysis = app(LocalSpeechAssessmentService::class)
 ->normalizeAssessment($transcription['pronunciation_analysis']?? null);
 ...
 $session,
 $answer->question,
 is_array($answer->observation_data?? null)? $answer->observation_data: [],
 $transcript,
 $metrics
 );
 $coachingFeedback = $this->safeCoachingFeedback(
 ...
 private function feedbackTranscriptionStatusFrom(array $transcription):?string
 {
 $status = $this->safeVoiceRecordingTranscriptionStatus($transcription['transcription_status']?? null);

 return $status === 'failed'? 'feedback_transcription_failed': $status;
 }

 private function uploadedFileForStoredVoiceRecording(InterviewAnswer $answer):?UploadedFile
 {
 ...
 $realPath,
 basename($path),
 $answer->voice_recording_mime_type?: 'audio/webm',
 null,
 true
 );
 }
 ...
 $assessmentAnswerText = $answerText!== ''? $answerText: $deliveryTranscript;
 ...
 try {
 $integrity = $this->answerIntegrityFrom($validated, $assessmentAnswerText, $transcriptTimeline);
 } catch (\Throwable $error) {
 ...
 InterviewSession $session,
 Question $question,?array $clientData,
 ...
 $alignment = is_array($answer->coaching_feedback?? null)? (array) data_get($answer->coaching_feedback, 'content_alignment', []): [];
 ...
 $excerpt = trim((string) ($alignment['evidence_quotes'][0]?? preg_replace('/\s+/u', ' ', mb_substr($this->answerContentForAssessment($answer), 0, 180))));
 $answerMissingPoints = is_array($alignment['missing_points']?? null)? array_values(array_filter($alignment['missing_points'])): ['A full coaching summary could not be generated for this answer.'];
 ...
 'what_worked' => $excerpt!== ''? 'The saved answer included this useful detail: "'.$excerpt.'".': 'The answer was saved and remains available for manual review.',
 ...
 $alignmentStatus = $isSkipped? 'skipped': ($wordCount < 10? 'insufficient_evidence': 'not_evaluated');
 $contentStatus = $wordCount === 0? 'unscored': ($wordCount < 10? 'limited_evidence': 'limited_evidence');
 ...
 'observation' => $deliveryStatus === 'measured'? 'Voice details were saved, but detailed speaking coaching could not finish for this request.': 'Speaking was not measured for this answer, so no filler, pace, or pause note was made.',
 ...
 'missing_points' => $isSkipped? ['No answer was sent, so the needed points are still missing.']: ['The app could not check how well this saved answer matched the question.'],
 'what_worked' => $answerExcerpt!== ''? 'Your answer was saved and can still be reviewed from the submitted text.': 'The response was saved as submitted.',
 ...
 {
 $expectedGuide = $question instanceof Question? ($question->expected_guide?? null): (is_array($question)? ($question['expected_guide']?? null): null);
 $mappedSkills = $question instanceof Question? ($question->mapped_skills?? []): (is_array($question)? ($question['mapped_skills']?? []): []);
 ...
 'expected_guide' => filled($expectedGuide)? mb_substr((string) $expectedGuide, 0, 800): null,
 ...
 InterviewSession $session,
 InterviewAnswer $answer,?string $feedback,
 string $stage
 ): array {
 try {
 ...
 return $this->fallbackAnswerEvidence($this->answerContentForAssessment($answer), $answer->question);
 }
 }
 ...
 InterviewAnswer $answer,
 array $evidence,
 string $stage,?string $fallback = null
 ): string {
 try {
 $revision = $assessment->groundedRevisionTemplate($this->answerContentForAssessment($answer), $evidence);
 if (trim($revision)!== '') {
 return $revision;
 }
 } catch (\Throwable $error) {
 ...
 return $fallback!== ''? $fallback: $this->fallbackGroundedRevisionTemplate($this->answerContentForAssessment($answer), $answer->question);
 }
 ...
 return [
 'headline' => 'Next focus: Fluency & Clarity',
 ...
 'priorities' => [[
 'skill' => 'Fluency & Clarity',
 'score' => (int) ($score->clarity_score?? 0),
 'task' => 'Practice one answer again and organize it into context, action, and true result.',
 ]],
 'recommended_paths' => [],
 'summary' => trim($feedback->improvement_suggestions?? '')?: 'Practice your weakest answer again and add only true details you can check.',
 ...
 {
 $isVoiceMode = $this->isVoiceResponseMode($input['response_mode']?? 'text');
 $voiceDuration = $isVoiceMode? $this->clampInt($input['voice_duration']?? 0, 0, 7200): 0;
 $hasVoiceEvidence = $isVoiceMode
 && $voiceDuration > 0
 && TranscriptService::wordCount($deliveryTranscript) > 0;
 $wpm = $hasVoiceEvidence? $this->clampInt(round((TranscriptService::wordCount($deliveryTranscript) / $voiceDuration) * 60), 0, 400): 0;
 $fillerWords = $hasVoiceEvidence? $this->clampInt(TranscriptService::countFillerWords($deliveryTranscript), 0, 500): 0;
 $pauseCount = $hasVoiceEvidence? min($this->clampInt($input['pause_count']?? 0, 0, 500), intdiv($voiceDuration, 2)): 0;
 ...
 InterviewAnswer $answer,?int $scoringConfidence = null,?InterviewSession $session = null,
 array $evaluation = []
 ): array {
 return array_merge([
 'answer_id' => $answer->id,
 ...
 $speechTranscript = array_key_exists('speech_transcript', $input)? (string) ($input['speech_transcript']?? ''): $answerText;
 ...
 if (! $this->deliveryTranscriptMatchesAnswer($speechTranscript, $answerText)) {
 return '';
 }
 ...
 InterviewSession $session,?Category $category,
 $questionText,
 string $difficulty,
 array $selectedTypes = [],
 int $index = 0,
 ...
 'type' => $questionType,
 'status' => 'active',
 'expected_guide' => filled($sourceMetadata['expected_guide']?? null)? $sourceMetadata['expected_guide']: $defaultCoaching['expected_guide'],
 'mapped_skills' =>! empty($sourceMetadata['mapped_skills']?? [])? $sourceMetadata['mapped_skills']: $defaultCoaching['mapped_skills'],
 ...
 return $record;
 }, $recommended, array_keys($recommended));
 }
 ...
 $questions = $this->limitedUniqueQuestionRecords(
 $difficultyMatched->concat($otherDifficulty),
 $limit
 );

 if ($questions->isEmpty() && empty($selectedTypes)) {
 $questions = $this->limitedUniqueQuestionRecords(collect($dataset['questions']?? []), $limit);
 }
 ...
 private function limitedUniqueQuestionRecords(Collection $questions, int $limit): Collection
 {
 $records = [];
 $seen = [];
 ...
 $key = $this->normalizedQuestionText($text);
 ...
 private function interviewFocus(?string $focus): string
 {
 $focus = trim((string) ($focus?: 'Job Interview'));
 $context = Str::contains(Str::lower($focus), ['philipp', 'filipino'])? $focus: "interview - {$focus}";
 ...
 return QuestionDatasetProvider::find($key)?? ($session->category? QuestionDatasetProvider::forCategory($session->category): null);
 }
 ...
 private function resolveInterviewCategory($categoryId = null):?Category
 {
 $activeCoreCategories = Category::where('status', 'active')->where('type', 'core');
 $category =! empty($categoryId)
 ? Category::where('status', 'active')->where('type', 'core')->find($categoryId)
 ...
 {
 $scenarioKind = $this->interviewScenarioKind($category);
 $vagueRecommendation = $this->vagueTargetRecommendation($position);

 if ($vagueRecommendation!== null) {
 return $this->vagueTargetRecommendationMessage($vagueRecommendation, $position, $scenarioKind);
 }
 ...
 private function vagueTargetRecommendation(string $position):?array
 {
 $target = $this->normalizeScenarioTargetText($position);
 ...
 'topic' => 'cleaning work',
 'examples' => 'Cleaner, Janitor, Housekeeping Attendant, or Janitorial Services',
 ],
 ];

 foreach ($recommendations as $recommendation) {
 if (in_array($target, $recommendation['terms'], true)) {
 ...
 private function targetScenarioKind(string $position):?string
 {
 $target = $this->normalizeScenarioTargetText($position);
 ...
 $needle = $this->normalizeScenarioTargetText((string) $phrase);
 ...
 private function normalizeScenarioTargetText(string $value): string
 {
 $normalized = preg_replace('/[^a-z0-9]+/', ' ', Str::lower($value))?? '';
 ...
 $focus = trim((string) ($session->interview_focus?: 'Job Interview'));
 $limit = max(1, min(30, $limit));

 $templates = [
 'Behavioral' => [
 "Tell me about a school, internship, freelance, or work project that best shows your readiness for a {$position} role in your target context.",
 ...
 'Situational' => [
 "If you joined as {$position} in a workplace and found unclear priorities in your first week, how would you respond?",
 ...
 "If an interviewer asked you, as {$position}, to explain a complex decision to a non-technical audience, how would you structure it?",
 ...
 'Technical' => [
 "Walk me through the technical strengths that make you a fit for a {$position} role in the target market.",
 ...
 'Personal' => [
 "Why are you interested in a {$position} role in your target context right now?",
 "What strengths would you bring as {$position}, and where are you still growing?",
 "As {$position}, how do you stay motivated when work becomes repetitive, high-volume, or ambiguous?",
 "What workplace setup helps you do your best work as {$position}: onsite, hybrid, remote, shifting, or regular hours?",
 "What do you want a interviewer to remember about your fit for {$position} after this conversation?",
 ],
 ];
 ...
 return [
 'source_name' => mb_substr($sourceName!== ''? $sourceName: 'Curated interview source', 0, 255),
 'source_url' => $sourceMetadata['source_url']?? null,
 ...
 {
 $metrics = [
 'Fluency & Clarity' => (int) ($score->clarity_score?? 0),
 'Answer Match' => (int) ($score->relevance_score?? 0),
 'Professional Tone' => (int) ($score->professionalism_score?? 0),
 ];
 ...
 $weakestSkill = array_key_first($weakest)?: 'Fluency & Clarity';
 $coachingPriorities = is_array($feedback->coaching_summary)? (array) ($feedback->coaching_summary['priority_actions']?? []): [];
 $topCoachingPriority = collect($coachingPriorities)->first(fn ($priority): bool => is_array($priority) && filled($priority['action']?? null));
 $headline = is_array($topCoachingPriority) && filled($topCoachingPriority['area']?? null)? 'Next focus: '.trim((string) $topCoachingPriority['area']): "Next focus: {$weakestSkill}";
 $summary = is_array($topCoachingPriority)? trim((string) ($topCoachingPriority['action']?? '')): '';
 ...
 'summary' => $summary!== ''? $summary: (trim($feedback->improvement_suggestions?? '')?: 'Practice your lowest-score answer again, then run a shorter targeted interview focused on that skill.'),
 'generated_at' => now()->toIso8601String(),
 ];
 }
 ...
 {
 return match ($skill) {
 'Fluency & Clarity' => 'Rewrite one answer in 60-90 seconds: context, main point, example, result.',
 ...
 if (in_array($weakestSkill, ['STAR Method', 'Fluency & Clarity', 'Answer Match'], true)) {
 return [
 ...
 {
 $gameLevelId = InterviewSession::hasColumn('game_level_id')? (int) ($session->game_level_id?? 0): 0;

 if (! InterviewSession::hasColumn('game_level_id')
 &&! $gameLevelId
 ...
 InterviewSession $session,?GameLevel $gameLevel,?string $gameStatus = null,?int $gameResultScore = null,
 array $resultContext = []
 ) {
 if ($gameLevel) {
 ...
 return redirect()->route('user.review', $session->id)->with('message', 'Interview completed! Here is your feedback.');
 }

 private function gameResultPayload(
 InterviewSession $session,
 GameLevel $gameLevel,?string $gameStatus,?int $gameResultScore,
 array $context = []
 ): array {
 $profile = Profile::firstOrCreate(['user_id' => Auth::id()]);
 ...
 $message = $passed? 'Victory! You cleared Level '.$gameLevel->level_number.' with '.$gameResultScore.'%.': 'You scored '.$gameResultScore.'% and need '.$gameLevel->required_score.'% to clear this level.';
 ...
 private function positionAwarePathLevelsFor(GameLevel $currentLevel): Collection
 {
 ...
 $relatedLevels = $selectedPosition!== ''? $challengePositions->relatedLevels($levels, $selectedPosition): $levels;
 ...
 'follow_up_question' => $starApplicable && $starScore < 70? 'For "'.$questionExcerpt.'", which missing STAR detail can you add using only true facts?': 'What true detail would make your response to "'.$questionExcerpt.'" more direct and complete?',
 ...
 private function answerContentForAssessment(InterviewAnswer $answer): string
 {
 ...
 $enabled = array_key_exists('enabled', $validated)? (bool) $validated['enabled']:! $session->is_public;
 ...
 $session->is_public? Auth::user()->name." enabled shared review link for interview session #{$session->id}.": Auth::user()->name." disabled shared review link for interview session #{$session->id}.",
 $request->ip(),
 false
 );
