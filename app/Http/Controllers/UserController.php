...
class UserController extends Controller
{
 ...
 'description' => 'Reduces the energy cost of all Interview Challenges by 1.',
 'cost' => 500,
 'type' => 'leadership',
 'icon' => 'fa-bolt',
 ],
 ...
 'description' => 'Starts every Interview Challenge with a +5 baseline score buffer.',
 'cost' => 500,
 'type' => 'communication',
 'icon' => 'fa-handshake',
 ],
 'time_extension' => [
 'name' => 'Time Extension',
 'description' => 'Grants an extra 30 seconds on all timed Interview Challenge levels.',
 'cost' => 500,
 ...
 'xp_boost' => [
 'name' => 'XP Boost',
 'description' => 'Permanently increases general XP earned from Interview Challenges by 20%.',
 'cost' => 500,
 'type' => 'technical',
 ...
 $scoreAverages = $this->averageScoreColumns($scoresQuery, [
 ...
 // Radar Data Averages
 $radarData = [
 'clarity' => round($scoreAverages['clarity_score']?? 0),
 ...
 })->with('session.score')->orderBy('created_at', 'desc')->first();
 ...
 if ($latestFeedback) {
 $latestS = $latestFeedback->session?->score;
 if ($latestS) {
 $skillsList = [
 ...
 // Notifications
 $userObj = Auth::user();
 $recentNotifications = $userObj->notifications? $userObj->notifications()->latest()->take(3)->get(): collect([]);
 ...
 'description' => 'Finish a practice interview to unlock readiness tracking',
 'progress' => 0,
 ],
 ];
 } else {
 ...
 $this->whereEscapedFeedbackLike($answerQuery, 'delivery_transcript', $like, 'or');
 ...
 default => 'Focus practice on structure, proof, and clarity.',
 };
 ...
 return collect([
 ['label' => 'Fluency & Clarity', 'column' => 'clarity_score', 'icon' => 'fa-comment-dots'],
 ...
 'value' => $value,
 'icon' => $metric['icon'],
 'color' => $value >= 80? '#10b981': ($value >= 60? '#2563eb': '#f59e0b'),
 ];
 })
 ->filter()
 ->values();
 ...
 $priorityActions = is_array($coachingFeedback['priority_actions']?? null)? $coachingFeedback['priority_actions']: [];
 ...
 $sessionDate = $sessionRecord->created_at? $sessionRecord->created_at->format('M d, Y'): 'selected date';
 ...
 'delta' => $delta,
 'label' => ($delta > 0? '+': '').$delta.'%',
 'trend_html' => $delta >= 0? "improved by <strong class='text-primary'>{$delta}%</strong>": "dropped by <strong class='text-danger'>".abs($delta).'%</strong>',
 ];
 }
 ...
 if ($delta < 0) {
 return 'Your latest score is lower than the previous one. Review the latest feedback before your next practice round.';
 }
 ...
 $averageScore = $scoredSessions->isNotEmpty()? (int) round($scoredSessions->avg()): null;
 $details = [];
 ...
 'last_activity_label' => $activityDates->isNotEmpty()? Carbon::parse($activityDates->last())->format('M d, Y'): 'No activity yet',
 ];
 }
 ...
 'total' => $part['total'],
 'percent' => $part['total'] > 0? $this->barWidth((int) round(($part['complete'] / $part['total']) * 100)): null,
 ];
 })
 ->values();
 ...
 $answerLabel = $analyzedAnswers > 0? ' across '.$analyzedAnswers.' analyzed answer'.($analyzedAnswers === 1? '': 's'): '';
 ...
 private function progressBadgesFor(array $badgesEarned, $sessions, object $starProgress, int $currentStreak, int $longestStreak): array
 {
 ...
 'description' => $completedSessions > 0? $completedSessions.' completed interview'.($completedSessions === 1? '': 's'): 'Complete 1 interview',
 ],
 (object) [
 'title' => '3-Day Streak',
 'icon' => 'fa-fire',
 ...
 'description' => $starProgress->has_data? $starProgress->overall_percent.'% STAR coverage': 'Use STAR effectively',
 ],
 (object) [
 'title' => '5 Interviews',
 'icon' => 'fa-list-check',
 'unlocked' => $hasBadge('5 Interviews') || $completedSessions >= 5,
 'description' => min($completedSessions, 5).'/5 completed interviews',
 ],
 ];
 }
 ...
 private function averageScoreColumns($query, array $columns): array
 {
 $columns = collect($columns)
 ->filter(fn (string $column): bool => Score::hasColumn($column))
 ->values();
 ...
 $select = $columns
 ->map(fn (string $column): string => "AVG({$column}) as {$column}")
 ->implode(', ');
 $row = (clone $query)->selectRaw($select)->first();

 return $columns
 ->mapWithKeys(fn (string $column): array => [$column => (float) ($row->{$column}?? 0)])
 ->all();
 }
 ...
 'user_id' => Auth::id(),
 'title' => $this->coachConversationTitle($message),
 ]);
 ...
 $isNewConversation? "{$user->name} started an AI coach conversation: {$conversation->title}.": "{$user->name} sent a new AI coach message in {$conversation->title}.",
 $request->ip(),
 false
 );
 ...
 if ($title === '') {
 return 'AI Coach Conversation';
 }

 return substr($title, 0, 30).(strlen($title) > 30? '...': '');
 }
 ...
 CoachLanguageService::FILIPINO => 'Para manatiling tumpak at kapaki-pakinabang, tumutulong lang ako sa interview preparation, resume o CV, skill certificates, job descriptions, at career coaching. Magpadala ng interview question, sagot, role, resume, certificate, o job description na gusto mong paghandaan.',
 CoachLanguageService::CEBUANO => 'Para magpabiling tukma ug mapuslanon, motabang lang ko sa interview preparation, resume o CV, skill certificates, job descriptions, ug career coaching. Ipadala ang interview question, tubag, role, resume, certificate, o job description nga gusto nimong praktisan.',
 CoachLanguageService::TAGLISH => 'Para accurate at useful, interview-related lang ang kaya kong tulungan: interview prep, resume/CV, skill certificates, job descriptions, and career coaching. Send an interview question, answer, target role, resume, certificate, or job description para ma-coach kita properly.',
 default => 'I can only help with interview preparation, resumes/CVs, skill certificates, job descriptions, and career coaching. Send an interview question, answer, target role, resume, certificate, or job description and I will help you from there.',
 };
 }
 ...
 'text_extraction_status' => $text!== ''? 'readable_text_extracted': 'no_readable_text_detected_or_extraction_unavailable',
 'readable_text' => $readableText,
 'interview_relevance' => $this->coachAttachmentLooksInterviewRelated($name, $text, $kind)? 'appears interview-related': 'not verified from readable text; ask the user how this supports interview preparation before treating it as evidence',
 ];
 }
 ...
 return @gzuncompress($stream)?: @gzinflate($stream)?: @gzdecode($stream)?: @gzinflate(substr($stream, 2))?: '';
 }
 ...
 public function learning(Request $request, ChallengePositionService $challengePositions)
 {
 GameSchema::ensure();
 ...
 $sessionCategoryId = (int) $request->session()->get('learning_challenge_category_id', 0);
 $requestedCategoryId = $request->has('category_id')? (int) $request->query('category_id'): null;

 if ($requestedCategoryId === null && $categories->count() > 0) {
 $redirectCategoryId = $sessionCategoryId > 0 && $availableCategoryIds->contains($sessionCategoryId)? $sessionCategoryId: $categories->first()->id;
 $redirectParams = ['category_id' => $redirectCategoryId];
 if ($request->boolean('choose_position')) {
 $redirectParams['choose_position'] = 1;
 }

 return redirect()->route('user.learning', $redirectParams);
 }
 ...
 ->orderBy('id', 'asc');
 $allVisibleGameLevels = $query->get();
 $allCategoryGameLevels = $requestedCategoryId!== null? $allVisibleGameLevels->where('category_id', $requestedCategoryId)->values(): $allVisibleGameLevels;

 $selectedChallengePosition = $this->selectedChallengePosition($request, $challengePositions);
 $gameLevels = $challengePositions->journeyLevels(
 $challengePositions->relatedLevels($allCategoryGameLevels, $selectedChallengePosition)
 );
 $hasPositionSpecificLevels = $selectedChallengePosition!== ''
 && $challengePositions->hasPositionSpecificMatches($allCategoryGameLevels, $selectedChallengePosition);
 ...
 || $selectedChallengePosition === ''
 || (! $request->session()->has('learning_challenge_position') &&! $request->has('position'))
 );

 $gameProgress = GameProgress::where('user_id', $user->id)->get()->keyBy('game_level_id');
 $selectedCategory = $requestedCategoryId!== null? $categories->firstWhere('id', $requestedCategoryId): null;

 return $this->mobileView('user.learning', compact(
 'profile',
 'gameLevels',
 'gameProgress',
 'categories',
 'selectedCategory',
 'selectedChallengePosition',
 'challengePositionOptions',
 'showPositionModal',
 'usingGeneralChallengeFallback'
 ));
 }

 public function updateLearningChallengePosition(
 Request $request,
 ChallengePositionService $challengePositions,
 LearningChallengeGenerationService $challengeGenerator
 )
 {
 GameSchema::ensure();
 ...
 'nullable',
 'integer',
 Rule::exists('categories', 'id')->where('status', 'active')->where('type', 'game'),
 ],
 'target_position' => ['required', 'string', 'max:255'],
 ]);

 $position = $challengePositions->clean($validated['target_position']?? null);
 if ($position === '') {
 return back()
 ->withErrors(['target_position' => 'Please enter your target position.'])
 ->withInput();
 }

 $request->session()->put('learning_challenge_position', $position);

 $user = Auth::user();
 if (Schema::hasColumn('users', 'target_position') && $challengePositions->clean($user->target_position?? null)!== $position) {
 $user->target_position = $position;
 $user->save();
 }
 ...
 private function selectedChallengePosition(Request $request, ChallengePositionService $challengePositions): string
 {
 $requestedPosition = $challengePositions->clean($request->query('position'));
 if ($requestedPosition!== '') {
 $request->session()->put('learning_challenge_position', $requestedPosition);

 return $requestedPosition;
 }

 $sessionPosition = $challengePositions->clean($request->session()->get('learning_challenge_position'));
 if ($sessionPosition!== '') {
 ...
 public function modules(Request $request, ChallengePositionService $challengePositions)
 {
 ...
 ->squish()
 ->limit(120, '');
 $selectedModulePosition = $this->selectedModulePosition($request, $challengePositions);
 ...
 $usingGeneralModuleFallback = false;
 ...
 $this->whereModulesRelatedToPosition($positionQuery, $selectedModulePosition);

 if ($positionQuery->exists()) {
 $query = $positionQuery;
 } else {
 $usingGeneralModuleFallback = (clone $query)->exists();
 }
 }
 ...
 'showModulePositionModal',
 'usingGeneralModuleFallback'
 ));
 }

 public function updateLearningModulePosition(
 Request $request,
 ChallengePositionService $challengePositions,
 LearningModuleGenerationService $moduleGenerator
 )
 {
 if (! Setting::enabled('ll_modules')) {
 return redirect()->route('dashboard')->with('error', 'Learning modules are currently disabled by the administrator.');
 ...
 $request->session()->put('learning_module_position', $position);
 $request->session()->put('learning_challenge_position', $position);
 ...
 'search' => (string) Str::of((string) ($validated['search']?? ''))->squish()->limit(120, ''),
 ])
 ...
 $message = $status === 'completed'? 'Module marked as completed. Nice work keeping your learning trail honest.': 'Module progress updated.';
 ...
 {
 $columns = ['title', 'description', 'category', 'difficulty', 'type', 'career_path', 'mapped_skills'];
 ...
 private function selectedModulePosition(Request $request, ChallengePositionService $challengePositions): string
 {
 $requestedPosition = $challengePositions->clean($request->query('position'));
 if ($requestedPosition!== '') {
 $request->session()->put('learning_module_position', $requestedPosition);

 return $requestedPosition;
 }

 $sessionPosition = $challengePositions->clean($request->session()->get('learning_module_position'));
 if ($sessionPosition!== '') {
 return $sessionPosition;
 }

 $challengePosition = $challengePositions->clean($request->session()->get('learning_challenge_position'));
 ...
 private function whereModulesRelatedToPosition($query, string $position): void
 {
 $terms = $this->modulePositionSearchTerms($position);

 $query->where(function ($positionQuery) use ($terms): void {
 foreach ($terms as $index => $term) {
 $pattern = $this->escapedModuleSearchPattern($term);
 $method = $index === 0? 'where': 'orWhere';

 $positionQuery->{$method}(function ($termQuery) use ($pattern): void {
 $this->whereEscapedModuleLike($termQuery, 'title', $pattern);
 $this->whereEscapedModuleLike($termQuery, 'description', $pattern, 'or');
 $this->whereEscapedModuleLike($termQuery, 'category', $pattern, 'or');
 $this->whereEscapedModuleLike($termQuery, 'career_path', $pattern, 'or');
 $this->whereEscapedModuleLike($termQuery, 'mapped_skills', $pattern, 'or');
 });
 }
 });
 }

 private function modulePositionSearchTerms(string $position): array
 {
 $normalizedPosition = (string) Str::of($position)->lower()->squish();
 $terms = collect([$position]);
 ...
 if (Str::contains($normalizedPosition, $group)) {
 $terms = $terms->merge($group);
 }
 }
 ...
 $tokens = preg_split('/\s+/', $normalizedPosition, -1, PREG_SPLIT_NO_EMPTY)?: [];

 $terms = $terms
 ->merge($tokens)
 ->map(fn (string $term): string => (string) Str::of($term)->squish())
 ->filter(fn (string $term): bool => strlen($term) >= 3 &&! in_array(Str::lower($term), $stopWords, true))
